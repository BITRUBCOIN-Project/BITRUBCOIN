...
TMPDIR_PREFIX = "bitrubcoin_func_test_"


class BitRubCoinTestFramework():
    """Base class for a bitrubcoin test script.

    Individual bitrubcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use bitrubcoin-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bitrubcoinds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a bitrubcoind test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple bitrubcoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitrubcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in bitrubcoin.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in bitrubcoin.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "bitrubcoind"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run bitrubcoinds:
    ...
    # BITRUBCOIN Specific TestFramework
    ...
    """
    Create a ProReg tx, which references an 100 BRC UTXO as collateral.
...
'''
BitRubCoinTestFramework extensions
'''
...
class BitRubCoinDMNTestFramework(BitRubCoinTestFramework):
...
# !TODO: remove after obsoleting legacy system
class BitRubCoinTier2TestFramework(BitRubCoinTestFramework):
