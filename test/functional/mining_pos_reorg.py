...
class ReorgStakeTest(BitRubCoinTestFramework):
    ...
    def check_money_supply(self, expected_brc):
        # verify that nodes have the expected BRC supply
        brc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(brc_supply, [DecimalAmt(expected_brc)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that BRC supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check BRC supply...")
