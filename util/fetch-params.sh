...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/BITRUBCOINParams"
    else
        PARAMS_DIR="$HOME/.bitrubcoin-params"
    fi
fi
...
Failed to fetch the BITRUBCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
BITRUBCOIN - fetch-params.sh

This script will fetch the BITRUBCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common BITRUBCOIN zkSNARK parameters. Note that it is
