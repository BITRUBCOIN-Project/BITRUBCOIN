...
*Setup instructions for a gitian build of BITRUBCOIN Core using a VM or physical system.*
...
to the BITRUBCOIN GitHub release page.
...
- [Building BITRUBCOIN Core](#building-bitrubcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bitrubcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bitrubcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bitrubcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bitrubcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bitrubcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bitrubcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/bitrubcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@bitrubcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bitrubcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bitrubcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bitrubcoin-osx-*-build.assert
```
