BITRUBCOIN Core version v5.4.0 is now available from: <https://github.com/bitrubcoin-project/bitrubcoin/releases>
...
If you are running an older version, shut it down. Wait until it has completely shut down, then run the installer (on Windows) or just copy over /Applications/BITRUBCOIN-Qt (on Mac) or bitrubcoind/bitrubcoin-qt (on Linux).
...
The `autocombinerewards` RPC command was soft-deprecated in v5.3.0 and replaced with explicit setter/getter commands `setautocombinethreshold`/`getautocombinethreshold`. BITRUBCOIN Core, by default, will no longer accept the `autocombinerewards` command, returning a deprecation error, unless the `bitrubcoind`/`bitrubcoin-qt` is started with the `-deprecatedrpc=autocombinerewards` option.
...
- #2579 bitrubcoin-cli better error handling + libevent RAII upstream backports (furszy)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/bitrubcoin-project-translations/), and every single person who tested the release candidates.
