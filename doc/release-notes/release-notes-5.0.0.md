BITRUBCOIN Core version *v5.0.0* is now available from:  <https://github.com/bitrubcoin-project/bitrubcoin/releases>
...
BITRUBCOIN Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, BITRUBCOIN Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run BITRUBCOIN v5.0.0:
...
#### Overview
Users will be able to protect their financial information by sending and receiving BRCs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://bitrubcoin.org .
...
* Top bar showing shield BRC balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield BRC balance.
...
See the release notes from [v4.2.0](https://github.com/BITRUBCOIN-Project/BITRUBCOIN/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed BRC supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in BRC
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in BRC
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zBRCsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield BRC)
  The `moneysupply` JSON return entry is the total BRC supply (the sum of all transparent and shield BRC)
...
| `getaccountaddress` | Returned the current BITRUBCOIN address associated with an internal account | Account system removal |
| `setaccount` | Associate a BITRUBCOIN address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given BITRUBCOIN address | Account system removal |
| `getaddressesbyaccount` | Returned a list of BITRUBCOIN addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a BITRUBCOIN address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a BITRUBCOIN address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zBRC balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zBRC mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zBRC mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zBRC amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zBRC to a BRC address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zBRC mints to a BRC address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zBRCs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzbrcseed` | Set the wallet's deterministic zBRC seed to a specific value | Zerocoin deprecation |
| `getzbrcseed` | Return the current deterministic zBRC seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zBRC seed | Zerocoin deprecation |
| `dzbrcstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzbrc` | Do a search for deterministically generated zBRC that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_bitrubcoin unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake bitrubcoind and unit tests: fixing missing MacOS framework. (furszy)
