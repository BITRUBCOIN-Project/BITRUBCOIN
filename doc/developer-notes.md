...
- [Development guidelines](#development-guidelines)
    - [General BITRUBCOIN Core](#general-bitrubcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BITRUBCOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BRCs (tBRC)" on the test network, if you
...
BITRUBCOIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bitrubcoin-project/bitrubcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bitrubcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bitrubcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/bitrubcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bitrubcoin.coverage/index.html`.
```

**Sanitizers**
BITRUBCOIN can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates BRCs (if wallet is enabled).
...
However, in open source software such as BITRUBCOIN Core, where everyone uses
...
pay attention to for reviewers of BITRUBCOIN Core code.

General BITRUBCOIN Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BITRUBCOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BITRUBCOIN Core so that they can be integrated
...
- src/chiabls
  - Upstream at https://github.com/BITRUBCOIN-Project/bls-signatures/ ; actively maintained by Core contributors, deviates from upstream https://github.com/Chia-Network/bls-signatures/
...
file descriptors it will cause problems with BITRUBCOIN's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:BITRUBCOIN-Project/BITRUBCOIN.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitrubcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitrubcoin-cli` and the GUI debug console use this table to determine how to
