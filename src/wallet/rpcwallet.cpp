...
// Copyright (c) 2015-2020 The BITRUBCOIN developers
...
UniValue delegatoradd(const JSONRPCRequest& request)
{
    ...
    if (!keyID)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from BITRUBCOIN address");
...
UniValue delegatorremove(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->HasAddressBook(*keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get BITRUBCOIN address from addressBook");
...
UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"       (string, required) The bitrubcoin address to be associated with a label.\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "2. \"amount\"      (numeric, required) The amount in BRC to send. eg 0.1\n"
            ...
            "                             The recipient will receive less BRCs than you enter in the amount field.\n"
...
static UniValue CreateColdStakeDelegation(CWallet* const pwallet, const UniValue& params, CTransactionRef& txNew, CReserveKey& reservekey)
{
    ...
    if (!IsValidDestination(stakeAddr) || !isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BITRUBCOIN staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BITRUBCOIN spending address");
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"staking_addr\"      (string, required) The bitrubcoin staking address to delegate.\n"
            "2. \"amount\"            (numeric, required) The amount in BRC to delegate for staking. eg 100\n"
            "3. \"owner_addr\"        (string, optional) The bitrubcoin address corresponding to the key that will be able to spend the stake.\n"
...
UniValue viewshieldtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                ...
                "      \"address\" : \"bitrubcoinaddress\",     (string) The BITRUBCOIN address involved in the transaction\n"
...
UniValue shieldsendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "      \"amount\":amount    (numeric, required) The numeric amount in " + "BRC" + " is the value\n"
                ...
                "                           Those recipients will receive less BRC than you enter in their corresponding amount field.\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty())
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"bitrubcoinaddress\",     (string) The bitrubcoin address\n"
            "      amount,                 (numeric) The amount in BRC\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nReturns the total amount received by the given bitrubcoinaddress in transactions with at least minconf confirmations.\n"
            ...
            "\nResult:\n"
            "amount   (numeric) The total amount in BRC received at this address.\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in BRC received for this label.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 4 ))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in BRC received for this wallet.\n"
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in BRC received for this wallet in P2CS contracts.\n"
...
static UniValue legacy_sendmany(CWallet* const pwallet, const UniValue& sendTo, int nMinDepth, std::string comment, bool fIncludeDelegated, const UniValue& subtractFeeFromAmount)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid BITRUBCOIN address: ")+name_);
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nSend to multiple destinations. Recipients are transparent or shield BITRUBCOIN addresses.\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The bitrubcoin address (either transparent or shield) is the key,\n"
            "                                     the numeric amount in BRC is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "\"bitrubcoinaddress\"  (string) A bitrubcoin address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in BRC received by the address\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"bitrubcoinaddress\",    (string) The bitrubcoin address of the transaction.\n"
            "    \"category\":\"category\",      (string) The transaction category (send|receive|orphan|immature|generate).\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BRC. This is negative for the 'send' category, and positive\n"
            "                                         for the 'receive' category,\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in BRC. This is negative and only available for the \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"address\":\"bitrubcoinaddress\",    (string) The bitrubcoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BRC. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in BRC. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in BRC\n"
            "  \"confirmations\" : n,     (numeric) The number of confirmations\n"
            "  \"bcconfirmations\" : n,             (numeric) DEPRECATED: Will be removed in a future release\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in BRC\n"
            "      \"label\" : \"label\",              (string) A comment for the address/transaction, if any\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending BRCs\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending BRCs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"bitrubcoinaddress\" \"test message\"") +
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid BITRUBCOIN address: ") + input.get_str());
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "     \"feeRate\"           (numeric, optional, default 0=estimate) Set a specific feerate (BRC per KB)\n"
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending BRCs.\n"
...
UniValue settxfee(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in BRC/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total BRC balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the BRC balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the BRC balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in BRC\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in BRC\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in BRC\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in BRC\n"
            "  \"txcount\": xxxxxxx,                      (numeric) the total number of transactions in the wallet\n"
            "  \"autocombine_enabled\": true|false,       (boolean) true if autocombine is enabled, otherwise false\n"
            "  \"autocombine_threshold\": x.xxx,          (numeric) the current autocombine threshold in BRC\n"
            ...
            "  \"paytxfee\": x.xxxx                       (numeric) the transaction fee configuration, set in BRC/kB\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in bitrubcoin.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in bitrubcoin.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) BRC value of the stakeable coins (minus reserve balance, if any)\n"
...
UniValue setstakesplitthreshold(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 BRC each)."
            + HelpRequiringPassphrase(pwallet) + "\n"

            "\nArguments:\n"
            "1. value                   (numeric, required) Threshold value (in BRC).\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("autocombinerewards")) {
        if (request.fHelp) {
            throw std::runtime_error("autocombinerewards (Deprecated, will be removed in v6.0. To use this command, start bitrubcoind with -deprecatedrpc=autocombinerewards)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "autocombinerewards is deprecated and will be removed in v6.0. To use this command, start bitrubcoind with -deprecatedrpc=autocombinerewards");
    }
    ...
    if (request.fHelp || request.params.empty() || (fEnable && request.params.size() != 2) || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same BITRUBCOIN address\n"
