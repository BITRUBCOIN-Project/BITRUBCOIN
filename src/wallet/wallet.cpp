...
/**
 * Fees smaller than this (in ubrc) are considered zero fee (for transaction creation)
                                      ...
                                      std::string& strError)
{
    ...
    if (txOut.nValue != consensus.nMNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s BRC", FormatMoney(Params().GetConsensus().nMNCollateralAmt));
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-bitrubcoin-address
...
bool CWallet::CreateCoinstakeOuts(const CBrcStake& stakeInput, std::vector<CTxOut>& vout, CAmount nTotal) const
{
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CBrcStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_BITRUBCOIN && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_BITRUBCOIN);
        }
