...
static const CBlockIndex* FindIndexFrom(uint32_t nChecksum, libzerocoin::CoinDenomination denom, int cpHeight)
{
    ...
    // Height limits are ensured by the contextual checks in NewZBrcStake
...
CLegacyZBrcStake* CLegacyZBrcStake::NewZBrcStake(const CTxIn& txin, int nHeight)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend()) {
        LogPrintf("%s: unable to initialize CLegacyZBrcStake from non zc-spend", __func__);
        return nullptr;
    }
    ...
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) ||
        ...
        LogPrint(BCLog::LEGACYZC, "%s : zBRC stake block: height %d outside range", __func__, nHeight);
        return nullptr;
    }

    // Check spend type
    libzerocoin::CoinSpend spend = ZBRCModule::TxInToZerocoinSpend(txin);
    ...
    if (_pindexFrom == nullptr) {
        LogPrintf("%s : Failed to find the block index for zbrc stake origin", __func__);
        return nullptr;
    }

    // All good
    return new CLegacyZBrcStake(_pindexFrom, _nChecksum, _denom, _hashSerial);
}

const CBlockIndex* CLegacyZBrcStake::GetIndexFrom() const
{
    return pindexFrom;
}

CAmount CLegacyZBrcStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZBrcStake::GetUniqueness() const
{
