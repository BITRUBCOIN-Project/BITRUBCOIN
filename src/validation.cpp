...
/** Fees smaller than this (in ubrc) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k BRC
...
{
    util::ThreadRename("bitrubcoin-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC_V2) &&
        ...
        if (pindex->nHeight % 50000 == 0) {
            ZBRCModule::CleanCoinSpendsCache();
        }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // BitRubCoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nBitRubCoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nBitRubCoinBadBlockBits)) {
            // accept BITRUBCOIN block minted with incorrect proof of work threshold
            return true;
        }
                                     ...
                                     std::set<CBigNum>& spent_serials)
{
    ...
    const bool zbrcActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC);
    const bool publicZbrcActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_PUBLIC);
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPublicSpend && (!publicZbrcActive || v5Active)) {
            ...
            if (isPrivZerocoinSpend && (!zbrcActive || publicZbrcActive)) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZBRCModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
                    spend = ZBRCModule::TxInToZerocoinSpend(in);
                }
        ...
        for (auto it = bl.vtx.rbegin(); it != bl.vtx.rend(); ++it) {
            ...
            for (const CTxIn& in: tx->vin) {
                ...
                if (!in.IsZerocoinSpend()) {
                    ...
                    // zerocoin serial
                    const CBigNum& s = ZBRCModule::TxInToZerocoinSpend(in).getCoinSerialNumber();
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, const FlatFilePos* dbp) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (isPoS) {
        ...
        if (coinstake_in.IsZerocoinSpend()) {
            libzerocoin::CoinSpend spend = ZBRCModule::TxInToZerocoinSpend(coinstake_in);
            if (!ContextualCheckZerocoinSpend(coinstake, &spend, pindex->nHeight)) {
                return state.DoS(100,error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                        coinstake.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbrc");
            }
        }
