...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select BRC Outputs to Spend" :
            "Select Shielded BRC to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::BRC;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show BITRUBCOIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
