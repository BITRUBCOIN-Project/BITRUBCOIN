...
#include "config/bitrubcoin-config.h"
#endif
...
#include "qt/bitrubcoin/splash.h"
#include "qt/bitrubcoin/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/bitrubcoin/governancemodel.h"
#include "qt/bitrubcoin/mnmodel.h"
...
{
    return QCoreApplication::translate("bitrubcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in bitrubcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in bitrubcoin.qrc)
...
/** Class encapsulating BITRUBCOIN Core startup and shutdown.
...
/** Main BITRUBCOIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (BITRUBCOINGUI)
...
{
    window = new BITRUBCOINGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &BITRUBCOINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &BITRUBCOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(BITRUBCOINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BITRUBCOINGUI::DEFAULT_WALLET);
        ...
        // BITRUBCOIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BITRUBCOINGUI::handlePaymentRequest);
        connect(window, &BITRUBCOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. BITRUBCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(bitrubcoin_locale);
    Q_INIT_RESOURCE(bitrubcoin);
    ...
    /// 6. Determine availability of data directory and parse bitrubcoin.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", BITRUBCOIN_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // bitrubcoin: links repeatedly have their payment requests routed to this process:
