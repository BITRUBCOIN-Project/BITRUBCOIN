...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BRC);
    ...
    if (settings.contains("nAnonymizeBitRubCoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizebitrubcoinamount", settings.value("nAnonymizeBitRubCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitrubcoin.cpp during shotdown to also remove the window positions
