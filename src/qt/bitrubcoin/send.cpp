...
#include "qt/bitrubcoin/send.h"
#include "qt/bitrubcoin/forms/ui_send.h"
#include "qt/bitrubcoin/addnewcontactdialog.h"
#include "qt/bitrubcoin/qtutils.h"
#include "qt/bitrubcoin/sendchangeaddressdialog.h"
#include "qt/bitrubcoin/optionbutton.h"
#include "qt/bitrubcoin/sendconfirmdialog.h"
#include "qt/bitrubcoin/guitransactionsutils.h"
#include "qt/bitrubcoin/loadingdialog.h"
...
SendWidget::SendWidget(BITRUBCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a BITRUBCOIN URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-brc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onBRCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onBRCSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent BRCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield BRCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent BRCs to shield."));
    }
}
...
void SendWidget::onBRCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto brcAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(brcAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(brcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(brcAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
