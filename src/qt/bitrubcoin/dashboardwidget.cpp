...
#include "qt/bitrubcoin/dashboardwidget.h"
#include "qt/bitrubcoin/forms/ui_dashboardwidget.h"
#include "qt/bitrubcoin/sendconfirmdialog.h"
#include "qt/bitrubcoin/txrow.h"
...
DashboardWidget::DashboardWidget(BITRUBCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareBrc, "square-chart-brc");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelBrc, "text-chart-brc");
    ...
    setCssProperty(ui->labelAmountBrc, "text-stake-brc-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &BITRUBCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isBrc = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair BRC, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            brc = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalBrc += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBrc.append(brc);
        chartData->valuesMN.append(mn);

        int max = std::max(brc, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesBrc);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBrc > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountBrc, "text-stake-brc");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountBrc, "text-stake-brc-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountBrc, ui->labelAmountMN});
    ui->labelAmountBrc->setText(GUIUtil::formatBalance(chartData->totalBrc, nDisplayUnit));
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZBRC) |
