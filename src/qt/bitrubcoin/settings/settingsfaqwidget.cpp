...
#include "qt/bitrubcoin/settings/settingsfaqwidget.h"
#include "qt/bitrubcoin/settings/forms/ui_settingsfaqwidget.h"
...
SettingsFaqWidget::SettingsFaqWidget(BITRUBCOINGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableBRC,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableBRC,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("BITRUBCOIN is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("BITRUBCOIN utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableBRCContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received BITRUBCOIN requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your BITRUBCOIN wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableBRC->setText(unspendableBRCContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of BITRUBCOIN with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("bitrubcoin-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.BITRUBCOIN.org'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(mnModel->getMNCollateralRequiredAmount(), BitcoinUnits::BRC)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in BITRUBCOIN Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the BITRUBCOIN network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableBRC, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableBRC);});
    ...
    if (parent)
        connect(parent, &BITRUBCOINGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
