...
#include "qt/bitrubcoin/forms/ui_snackbar.h"
#include "qt/bitrubcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BITRUBCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BITRUBCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
