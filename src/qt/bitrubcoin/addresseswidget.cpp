...
#include "qt/bitrubcoin/forms/ui_addresseswidget.h"
#include "qt/bitrubcoin/addressfilterproxymodel.h"
#include "qt/bitrubcoin/addresslabelrow.h"
#include "qt/bitrubcoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(BITRUBCOINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto brcAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(brcAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(brcAdd);
        if (walletModel->updateAddressBookLabels(brcAdd, label.toUtf8().constData(),
