...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZbrc)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZbrc)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZbrc);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZbrc)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZbrc)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZbrc);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter BITRUBCOIN address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no BITRUBCOIN: URI
...
{
    // Convert bitrubcoin:// to bitrubcoin:
    //
    //    Cannot handle this later, because bitrubcoin:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BRC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", BITRUBCOIN_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        ...
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BITRUBCOIN (regtest).lnk";
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a bitrubcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=BITRUBCOIN (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=BITRUBCOIN (regtest)\n";
        else
