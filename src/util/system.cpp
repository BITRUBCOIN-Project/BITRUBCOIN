...
const char * const BITRUBCOIN_CONF_FILENAME = "bitrubcoin.conf";
const char * const BITRUBCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";


// BITRUBCOIN only features
// Masternode
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitrubcoind -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\BITRUBCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\BITRUBCOIN
// Mac: ~/Library/Application Support/BITRUBCOIN
// Unix: ~/.bitrubcoin
#ifdef WIN32
    // Windows
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BITRUBCOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BITRUBCOINParams
    // Mac: ~/Library/Application Support/BITRUBCOINParams
    // Unix: ~/.bitrubcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BITRUBCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "BITRUBCOINParams";
#else
    // Unix
    return pathRet / ".bitrubcoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/bitrubcoin/sapling-spend.params";
        sapling_output = "/usr/share/bitrubcoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/bitrubcoin/sapling-spend.params";
            sapling_output = "/usr/local/share/bitrubcoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", BITRUBCOIN_MASTERNODE_CONF_FILENAME));
