...
// Copyright (c) 2015-2021 The BITRUBCOIN developers
 ...
 */
const char * const BITRUBCOIN_PID_FILENAME = "bitrubcoin.pid";

fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", BITRUBCOIN_PID_FILENAME));
...
void Shutdown()
{
    ...
    util::ThreadRename("bitrubcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", BITRUBCOIN_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", BITRUBCOIN_PID_FILENAME));
#endif
...
{
    util::ThreadRename("bitrubcoin-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that BITRUBCOIN is running in a usable environment with all
...
{
    // Make sure only a single BITRUBCOIN process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BITRUBCOIN_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if BITRUBCOIN is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if BITRUBCOIN is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //BITRUBCOIN specific: zerocoin and spork DB's
                ...
                // BITRUBCOIN: load previous sessions sporks if we have them.
