...
UniValue validateaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"bitrubcoinaddress\"\n"
            "\nReturn information about the given bitrubcoin address.\n"

            "\nArguments:\n"
            "1. \"bitrubcoinaddress\"     (string, required) The bitrubcoin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"bitrubcoinaddress\",    (string) The bitrubcoin address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address -only if is standard address-\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for BITRUBCOIN cold staking -only if is standard address-\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BITRUBCOIN address and we have full public key:
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"bitrubcoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"bitrubcoinaddress\"  (string, required) The bitrubcoin address to use for the signature.\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ --------
    ...
    { "util",               "validateaddress",        &validateaddress,        true,  {"bitrubcoinaddress"} }, /* uses wallet if enabled */
    { "util",               "verifymessage",          &verifymessage,          true,  {"bitrubcoinaddress","signature","message"} },
