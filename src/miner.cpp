...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("BITRUBCOINMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("BITRUBCOINMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("BITRUBCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("bitrubcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BITRUBCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("BITRUBCOINMiner exception");
    } catch (...) {
        LogPrintf("BITRUBCOINMiner exception");
    }

    LogPrintf("BITRUBCOINMiner exiting\n");
}
